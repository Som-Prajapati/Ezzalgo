...
const Controls: React.FC<ControlsProps> = ({
  ...
  const userInput = useRef<string>(array.join(", "));
  ...
  // Update array input when array prop changes
  useEffect(() => {
    userInput.current = array.join(", ");
    if (isMounted) {
      const inputElement = document.querySelector(
        'input[placeholder="Enter numbers separated by commas"]'
      ) as HTMLInputElement;
...
//       if (newIsPlaying) {
//         if (playFunction.current) playFunction.current();
//       } else {
//         if (pauseFunction.current) pauseFunction.current();
//       }
//       return newIsPlaying;
//     });
//   };
